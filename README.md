...
Place your `.p12` keystore file at `keystore/release-signing.p12` (or update `storeFile` to match your path).
...
> **Nothing in the Kotlin source needs editing** — all three core values are injected at build time via `BuildConfig` and `resValue`.
...
- The `keystore/` directory is tracked by git; committing your keystore file there is optional but convenient for private repos
